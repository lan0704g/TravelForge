...
from travel_forge.agents.constraint_checker import format_constraint_check, validate_constraints
from travel_forge.agents.extractor_agent import (
    classify_intent,
    extract_trip_request,
    resolve_contextual_trip_update,
)
from travel_forge.config import OUTPUT_DIR
from travel_forge.memory import TravelForgeMemory
from travel_forge.models import get_chat_model, get_fast_model, get_reflection_model
from travel_forge.rag.retriever import search_travel_docs
from travel_forge.schemas import TripRequest
from travel_forge.tools.budget_tools import estimate_trip_budget
from travel_forge.tools.gaode_mcp_tools import (
    distance_via_gaode,
    geocode_via_gaode,
    get_weather_via_gaode,
    search_poi_via_gaode,
)
from travel_forge.tools.hotel_mcp_tools import query_hotel_reference_price
from travel_forge.tools.runtime_tools import calculate, current_time_text, get_current_time
...
GENERAL_CHAT_PROMPT = """
你是 Agent-TravelForge，一个懂旅行规划的智能助手。
...
DRAFT_PROMPT = """
你是 Agent-TravelForge 的行程生成 Agent。
...
你是 Agent-TravelForge 的正式行程报告生成 Agent。
...
REFLECT_JSON_PROMPT = """
你是 Agent-TravelForge 的 Reflection Agent。
请审查行程初稿是否需要优化。
...
REFINE_PROMPT = """
你是 Agent-TravelForge 的 Refine Agent。
...
REVISION_PROMPT = """
你是 Agent-TravelForge 的行程修订 Agent。
