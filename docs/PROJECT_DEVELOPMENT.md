# Agent-TravelForge 项目详细开发文档

## 1. 项目定位

Agent-TravelForge 是一个基于 LangChain、RAG、本地向量模型和 MCP 工具的智能出行策划与执行 Agent。
...
```text
python -m travel_forge.app.list_mcp_tools
```

查看 MCP 工具参数：

```text
python -m travel_forge.app.inspect_mcp_tools
```
...
UI 不依赖 CLI。CLI 和 UI 都只是入口层，核心都调用 `TravelForgeConversationAgent`。
...
本地 BGE 模型封装在 `travel_forge/rag/embeddings.py`，避免依赖 OpenAI Embeddings。
...
文件：`travel_forge/agents/conversation_agent.py`

主入口是 `TravelForgeConversationAgent`。
...
文件：`travel_forge/agents/extractor_agent.py`
...
文件：`travel_forge/agents/reflection_agent.py`
